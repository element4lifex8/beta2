...
class Helpers{
    ...
    fileprivate let sharedLoginType = UserDefaults.standard
    
    //retrieve the current app user from NSUserDefaults
    var loginType: userType {
        get{
            return (sharedLoginType.object(forKey: loginTypeDefaultKey) as? userType)!
        }
        set
        {
            sharedLoginType.set(newValue, forKey: loginTypeDefaultKey)
        }
    }
